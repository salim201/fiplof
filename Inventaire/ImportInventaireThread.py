...
class ImportInventaireThread(QThread):
    def __init__(self, filenames, connection, refimport = 0, steps = None):
        ...
        self.steps = steps
        self.logger = xlsLogger.xlsLogger("Import_Inventaire_Parcelle")
        ...
        for data in csv_personne:
            ...
            if line > 0:
                ...
                if datas[12].strip().decode('utf-8').strip().lower() != "celibataire":
                    sit_matri = 1
                elif datas[12].strip().decode('utf-8').strip().lower() != "marie":
                    sit_matri = 2
                ...
                if idParcelle is not None:
                    ...
                    try:
                        ...
                        cursor.execute('UPDATE parcelle_d SET region = %s, district = %s, commune = %s, id_commune = %s, idhameau = %s, consistance = %s, categorie = %s, date_inventaire = %s, sujet_demande = %s, '
                                       'user_import_inv = %s, date_import_inv = %s, inventaire = True, ref_import = %s,  fkt = %s'
                                           ' WHERE gid = %s',(region, district, commune, idCommune, idHameau, consistance, categorie, dateinventaire, sujetDemande, user_import, dateImport, self.refimport,str(fokontany).strip(),idParcelle ))
                        self.connection.commit()
                    ...
                    if self.select("parcelle_d", "idcertificat", where) is not None:
                        ...
                        try:
                            ...
                            if err.pgcode == "23505":
                                ...
                                self.connection.rollback()
                        dataToLog.append(
                            {"code_parcelle": codeParcelle, "idpers_xl": str(id_pes_xl),
                             "etat_insertion": etatInsertion,
                             "erreur": erreur})
