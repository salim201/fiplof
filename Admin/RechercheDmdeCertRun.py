...
class RechercheDmdeCertRun(QDialog):
    ...
    def initActions(self):
        ...
        self.ui.btnDelDemande.clicked.connect(self.confirmDelDemande)
    ...
    def confirmDelDemande(self):

        print("id parcelle ")
        print(self.idparcelle) # vs None


        print(" demande ")
        print(self.numDemande) # Vs 0

        if(self.numDemande is None):
            QMessageBox.critical(self, "Erreur Opposition",
                                 u"Veuillez selectionner une ligne")
            return
        try:
            print(" Delete sql")
            cursor = self.connection.cursor()

            if self.idparcelle != 0:
                print "delete"

                cursor.execute("SELECT *   FROM demande  WHERE numdemande=%s", [str(self.numDemande)])
                res = cursor.fetchone()
                cursor.execute("delete from  avoir_demande WHERE iddemande =%s", [int(res[0])])
                cursor.execute("delete from  demande WHERE numdemande=%s", [str(self.numDemande)])
                cursor.execute("delete from  limitesparcelle WHERE idparcelle=%s", [str(self.idparcelle)])
                cursor.execute("delete from  proprietaireparcelle WHERE idparcelle=%s", [int(self.idparcelle)])
                cursor.execute("delete from  autrecharge WHERE idparcelle=%s", [int(self.idparcelle)])
                cursor.execute("delete from  autrechargesparcelle_d WHERE idparcelle=%s", [int(self.idparcelle)])
                cursor.execute("delete from  oppositions WHERE gid=%s", [int(self.idparcelle)])
                cursor.execute("delete from  hypothequeparcelle_d WHERE idparcelle=%s", [int(self.idparcelle)])
                cursor.execute("delete from  parcelle_d WHERE gid=%s", [str(self.idparcelle)])

                cursor.execute("SELECT *   FROM certificat  WHERE numerodemande=%s", [str(self.numDemande)])
                res = cursor.fetchone()
                if(res is not None):
                    print(" CF")
                else :
    ...
    def ouvrirChoixSurParcelle(self):
        ...
        try :
            if self.choix.creation.getEtatOpposition():
                reply = QMessageBox.critical(self, "Erreur Opposition",
    ...
    def rechercher(self, data):
        ...
        flag = 0
        listeParams =[]
        SQL = "SELECT DISTINCT pd.gid, d.numdemande, d.datedemande, d.datereconnaissance, pd.cout, d.nomdemandeur, d.gid FROM parcelle_d pd, demande d WHERE pd.gid = d.gid  "
        SQL_count = "SELECT DISTINCT COUNT(*) FROM parcelle_d pd, demande d WHERE pd.gid = d.gid  "
