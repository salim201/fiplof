...
class DbExportThread(QThread):
    ...
    def run_import(self):
        if self.isImportData:
            ...
            tab_name_of_file = str(self.filename).split("/")
            if len(tab_name_of_file) <= 1:
                tab_name_of_file = str(self.filename).split("\\")
            name_of_file = str(tab_name_of_file[len(tab_name_of_file) - 1]).replace(".7z",".sql")
            print ("tmp dir *******************************************")
            print (self.tmp_dir + " " + name_of_file)
            self.filename = os.path.join(self.tmp_dir,
                                          name_of_file)
    ...
    def unCompressFile(self):
        ...
        self.tmp_dir = str(tempfile.gettempdir())
        proc = subprocess.Popen(
            [
                "%s" % (file_to_exec,),
                "e", self.filename,
    ...
    def create_batfile(self):
        ...
        if not self.isImportData:
            ...
            "\n-- Name: "+self.nomBase+"; Type: DATABASE; Schema: public; Owner: "+self.db_config.db_user+""\
            "\n--" \
