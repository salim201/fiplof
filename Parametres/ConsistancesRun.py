...
class ConsistancesRun(Qt.QDialog):
    def __init__(self, connection):
        ...
        self.pgsql.addColumn("libelleconsistance", "Categorie")
        self.pgsql.addColumn("parcelleoubatiment", "Karazan-tany")
        self.ui = Ui_Dialog()
    ...
    def nouveau(self):
        dialog = EditConsistanceRun(self.connection)
        if dialog.exec_():
            self.refresh()
    ...
    def modifier(self):
        ...
        if uid is None:
            return
        dialog = EditConsistanceRun(self.connection, uid)
        if dialog.exec_():
            self.refresh()
