...
#
# Created: Tue Feb 13 19:51:12 2018
...
from DetailsDemandes import Ui_CreationDemande

# create the dialog for qgsPlof
class DetailsDemandeRunn(QtGui.QDialog):
  def __init__(self,parent):
    QtGui.QDialog.__init__(self)
    from Configuration import DbConfig
    self.setModal(True)
    self.ui = Ui_CreationDemande ()
    ...
    self.initActions()
    self.getDemande()
    self.ui.pushButton_6.clicked.connect(self.callDemandeur)
    self.ui.pushButton.clicked.connect(self.callOpposition)
    self.ui.pushButton_2.clicked.connect(self.callRejectDemande)
    self.ui.fokontanyComboBox.currentIndexChanged.connect(self.fillFokontany)
    self.ui.dateDeReconnaissanceLineEdit.dateChanged.connect(self.evaluateDateReconnaissance)
  ...
  def init_canvas(self):
    ...
    # self.idparcelle;
    # add canvas  ad form
    self.ui.horizontalLayout_3.addWidget(self.cv)
    self.cv.show()
  ...
  def evaluateDateReconnaissance(self):
    ...
    a = self.ui.dateDemandeLineEdit.text()
    a = a.split("/")
    a = date(int(a[2]), int(a[1]), int(a[0]))

    b = self.ui.dateDeReconnaissanceLineEdit.text()
    b = b.split("/")
    ...
    if self.ddDate < 15:
      QMessageBox.critical(self.ui.dateDeReconnaissanceLineEdit, "Erreur",
  ...
  def initActions(self):
    ...
    if self.edition == 2:
      self.ui.pushButton_4.clicked.connect(self.updateGeometrie)
      self.ui.lineEditDistrict.setEnabled(False)
      self.ui.coutLineEdit.setReadOnly(True)
      self.ui.lineEditRegion.setReadOnly(True)
      self.ui.dateDemandeLineEdit.setReadOnly(True)
      self.ui.dateDeReconnaissanceLineEdit.setReadOnly(True)
      self.ui.lineEditDemande.setReadOnly(True)
      self.ui.pushButton_2.setEnabled(False)
      self.ui.pushButton.setEnabled(False)
      self.ui.pushButton_6.setEnabled(False)
      self.ui.communeComboBox.setReadOnly(True)
      self.ui.fokontanyComboBox.setDisabled(True)
      self.ui.pushButton_4.setText(QtGui.QApplication.translate("CreationDemande", "Modifier la Geometrie", None, QtGui.QApplication.UnicodeUTF8))
    ...
    if self.edition == 1:
      self.ui.pushButton_4.clicked.connect(self.updateInfos)
      self.ui.pushButton_4.setText(QtGui.QApplication.translate("CreationDemande", "Modifier les Informations", None,QtGui.QApplication.UnicodeUTF8))
    ...
    if self.edition == 0 :
      ...
      self.ui.lineEditDemande.setEnabled(True)
      self.ui.lineEditDemande.setReadOnly(True)
      self.ui.coutLineEdit.setReadOnly(True)
      self.ui.lineEditRegion.setReadOnly(True)
      self.ui.lineEditDistrict.setReadOnly(True)
      self.ui.communeComboBox.setEnabled(True)
      self.ui.communeComboBox.setReadOnly(True)
      self.ui.dateDeReconnaissanceLineEdit.setReadOnly(True)
      self.ui.dateDemandeLineEdit.setReadOnly(True)
  ...
  def generateDemandeKey(self):
    ...
    self.ui.lineEditDemande.setText(
  ...
  def getDemande(self):
    ...
    # cursor.execute("SELECT *   FROM oppositions  WHERE gid=%s", [self.gid])
    # opposition = cursor.fetchone()
    ...
    if (len(res) >= 1):
      ...
      self.ui.dateDemandeLineEdit.setDate(dateDemande)
      ...
      self.ui.dateDeReconnaissanceLineEdit.setDate(dateReconnaissance)

      self.iddemandeForJournal = res[0]  # Recuperation de l'id de la table demande
      self.ui.lineEditDemande.setText(str(res[2]))
      self.iddemande = int(res[0])
      self.ui.coutLineEdit.setText(str(res[19]))
      self.ui.lineEditRegion.setText(str(res[11]))
      self.ui.lineEditDistrict.setText(str(res[12]))
      self.ui.communeComboBox.setText(str(commune[3]))
      self.loadFokontany(int(commune[0]))
      self.loadConsistance()
      self.filenamepreview = self.preview()
      #self.ui.label_2.setPixmap(QtGui.QPixmap(self.filenamepreview))
  ...
  def callRejectDemande(self):
    print "Reject demande"
    from RejetDemandeRun import RejetDemandeRun
    rejet = RejetDemandeRun(self)
    rejet.show()
    result = rejet.exec_()
  ...
  def callOpposition(self):
    ...
    print
    "call opposition "
    numDmd = str(self.ui.lineEditDemande.text())
  ...
  def updateGeometrie(self):
    ...
    journal.inserToJournal(globalvars.id_user, self.iddemandeForJournal, u"Demande",
                           u"Edition de la géometrie d'une demande")
  ...
  def updateInfos(self):
    ...
    if self.ddDate < 15:
      ...
      #self.ui.dateDeReconnaissanceLineEdit.setFocus(Qt.Qt.OtherFocusReason)
      return
    ...
    self.numDemande = numDmd
    dateDemande = self.ui.dateDemandeLineEdit.text()
    dtDmande = dateDemande.split('/')

    dateReconnaissance = self.ui.dateDeReconnaissanceLineEdit.text()
    dtReconnaissance = dateReconnaissance.split('/')

    cout = int(self.ui.coutLineEdit.text())
    region = str(self.ui.lineEditRegion.text())
    district = str(self.ui.lineEditDistrict.text())
