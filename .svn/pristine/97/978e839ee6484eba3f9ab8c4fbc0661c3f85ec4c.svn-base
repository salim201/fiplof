...
class PersonneMoraleRun(QDialog):
    def __init__(self, connection, parent = None):
        ...
        self.idRepresentant = None
        self.pp = ListePersonnePqueRun(self.connection)
        self.initActions()
        self.initDB()
        self.consulter = 0
        self.isValid = True
 ...
 #   def enregAndClose(self):
    ...
    def initActions(self):
        ...
        self.ui.pushButtonListePP.clicked.connect(self.openPP)
        self.pp.ui.btnSelectionner.clicked.connect(self.getIdPP)

    def getNomRepresentant(self):
        cur = self.connection.cursor()
        nom = ""
        try:
            cur.execute("SELECT CONCAT(pp.nompersonne,' ', pp.prenompersonne) FROM personne pp WHERE pp.idpersonne = %s", (self.idRepresentant,))
            res = cur.fetchone()
    ...
    def getIdPP(self):
        print ("in get idpp")
        self.idRepresentant =  self.pp.getIdPersonne()
        self.ui.lineEditNomRepresentant.setText(self.getNomRepresentant())
        self.pp.close()
    ...
    def readInput(self):
        if self.consulter == 1:
            ...
            if self.ui.lineEditNom.text() != "":
                ...
                self.isValid = False
            if self.idRepresentant is None:
                self.missedFields.append("Nom representant")
                self.isValid = False
    ...
    def insertData(self, data):
        if self.consulter == 0:
            print(data)
            try:
                self.cur.execute("INSERT INTO personnemorale (denomination, datecreation, siege, observation, idtype, idrepresentant) VALUES (%s, %s, %s, %s, %s, %s) returning idpersonnemorale",(data['denomination'], data['datecreation'], data['siege'], data['obs'], data['type'], self.idRepresentant))
            ...
            try:
                self.cur.execute("UPDATE personnemorale SET denomination = %s, datecreation = %s, siege = %s, observation = %s, idtype = %s, idrepresentant = %s WHERE idpersonnemorale = %s returning idpersonnemorale",(data['denomination'], data['datecreation'], data['siege'], data['obs'], data['type'], self.idRepresentant, self.idPersonne))
                self.connection.commit()
                #self.idPersonne = self.cur.fetchone()
                self.idPersonne = self.cur.fetchone()
                return self.idPersonne
