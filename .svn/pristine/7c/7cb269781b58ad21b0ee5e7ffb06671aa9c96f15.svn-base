...
class AutoRestoreThread(QThread):
    ...
    def __init__(self, pgdump, dirname, connection, parent, isOnlineRestore = False):
        ...
        self.isOnlineRestore = isOnlineRestore
        if isOnlineRestore:
    ...
    def run(self):
        ...
        if most_recent_file == None:
            ...
            if self.dbHost == '' or self.dbPort == '' or self.dbPass == '' or self.dbUser == '' or self.nomBase == '':
                self.errorAddedSignal.emit(u"Certains parametres nécéssaires à la restauration sont manquantes. Veuillez verifier les options d'interconnexion.")
                return
    ...
    def run_dump(self):
        ...
        try:
            test_connection = psycopg2.connect(database=self.nomBase,
                                               user=self.dbUser,
    ...
    def run_import(self):
        ...
        while True:
            ...
            if output:
                if ('PGPASSWORD' not in output.strip()):
                    self.messageAddedSignal.emit(output.strip())
                    print (output.strip())
                if (('psql:' in output.strip()) and ('timed out' in output.strip() or 'FATAL' in output.strip() or 'Unknown host' in output.strip())):
        ...
        proc.poll()
        os.remove(bat)
        if self.isOnlineRestore:
            self.updateDateLastRestore()
        self.doneSignal.emit()
    ...
    def updateDateLastRestore(self):
        try:
            cur = self.connection.cursor()
            ...
            print ('Erreur update configuratio: '  + str(err))
