...
class ImporterThreadIpss(QThread):
    ...
    def getIdFkt(self, codefkt):
        ...
        code_fkt = int(codefkt)
        try:
            cur.execute('SELECT idfokontany from fokontany WHERE CAST(codefokontany as integer) = %s', (code_fkt,))
            res = cur.fetchone()
            return res[0]
        except Exception as err:
            print(err)
    ...
    def importHameau(self):
        ...
        try:
            ...
            print(err)
        # print(fileDirPath)
        filePath = os.path.join(fileDirPath, "LISTE_HAM_ET_FKT.xls")
        print (filePath)
        try:
        ...
        while line < sheet.nrows:
            ...
            if line >= 1:
                ...
                nomHameau = str(sheet.cell_value(line, 2)).decode('utf-8')
                codeHameau = str(sheet.cell_value(line, 3)).decode('utf-8')
                codeFkt = str(sheet.cell_value(line, 4)).decode('utf-8')
    ...
    def importConsistance(self):
        ...
        while line < sheet.nrows:
            ...
            if line >= 1:
                ...
                consistance_parc = str(sheet.cell_value(line, 1)).decode('utf-8')
                consistance_bat = str(sheet.cell_value(line, 2)).decode('utf-8')
                print 'eto'
                cursor = self.connection.cursor()
                try:
    ...
    def importPersonnePhysique(self):
        ...
        try:
            ...
            print(err)
        # print(fileDirPath)
        filePath = os.path.join(fileDirPath, "PP_to_Import.xls")
        print (filePath)
        try:
            wb = open_workbook(filePath)
        except Exception as err:
