...
from .Hameaux import Ui_Dialog
from .EditHameauRun import EditHameauRun
from PgCrud import PgSql, PgColumn

class HameauxRun(Qt.QDialog):
    def __init__(self, connection):
        ...
        self.pgsql.join("fokontany", "hameau.idfokontany = fokontany.idfokontany")
        self.pgsql.addColumn("idhameau", "Id", True, PgColumn.ColumnType.INTEGER)
        self.pgsql.addColumn("nomfokontany", "Fokontany")
        self.pgsql.addColumn("codehameau", "Code")
        self.pgsql.addColumn("nomhameau", "Nom")
        self.refresh()
        self.initActions()
    ...
    def nouveau(self):
        dialog = EditHameauRun(self.connection)
        if dialog.exec_():
            self.refresh()
    ...
    def modifier(self):
        ...
        if uid is None:
            return
        dialog = EditHameauRun(self.connection, uid)
        if dialog.exec_():
            self.refresh()
    ...
    def supprimer(self):
        ...
        if uid is None:
            return
        reply = Qt.QMessageBox.question(self, "Confirm", "Voulez-vous supprimer ce hameau ?",
