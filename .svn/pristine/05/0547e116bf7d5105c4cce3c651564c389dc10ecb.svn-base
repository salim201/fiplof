...
class ImportDataRLThread(QThread):
    def __init__(self, filenames, connection, refimport = 0, steps = None, idfkt = 0, idham = None, region = ''):
        ...
        self.idHam = idham
        self.region = region
        self.logger = xlsLogger.xlsLogger("Import_Data_RL")
        self.numdemande, self.etatInsertion, self.erreur = '', '', ''
    ...
    def run(self):
        ...
        for step in self.steps:
            if str(step).strip() == u"Fichier de forme":
                # IMPORT shapefile
                self.emit(SIGNAL("stepInit(int)"), self.current_step + 1)
                print("Debut import shapefile")
                self.importParcelle()
                print("Fin import shapefile")
            ...
            if str(step).strip() == u"Données RL":
                #IMPORT listing
                self.emit(SIGNAL("stepInit(int)"), self.current_step + 1)
                print("Debut import Listing")
                self.importListing()
                print("Fin import Personne Listing")
                if str(step).strip() == str(self.steps[len(self.steps) - 1]).strip():
                    self.logger.write()
                self.emit(SIGNAL("stepDone(int)"), self.current_step)
                #FIN IMPORT Listing
    ...
    def importListing(self):
        ...
        if sh_demande is not None:
            ...
            while line < sh_demande.nrows:
                ...
                if line >= 1:
                    ...
                    dataParcelle['fokontany'] = str(sh_demande.cell_value(line, 3)).strip().upper().decode('utf-8')
                    dataParcelle['hameau'] = str(sh_demande.cell_value(line, 4)).strip().upper().decode('utf-8')
                    dataParcelle['lieudit'] = str(sh_demande.cell_value(line, 5)).strip()
                    dataParcelle['codePlanche'] = str(sh_demande.cell_value(line, 6)).strip()
                    dataParcelle['collecteur_demande'] = str(sh_demande.cell_value(line, 7)).strip()
                    dataPersonne['nom_prenom'] = str(sh_demande.cell_value(line, 8)).strip().upper()
                    ...
                    #Check commune
                    nomCommuneFromDb = self.select('commune', 'UPPER(TRIM(nomcommune))',
                                                     "UPPER(TRIM(nomcommune)) = '" + dataParcelle[
                                                         'commune'] + "' and idcommune = " + str(
                    ...
                    if nomCommuneFromDb is None:
                        ...
                        #Check Hameau
                            ...
                            dataPersonne['lieu_pi'] = str(sh_demande.cell_value(line,15)).strip()
                            dataPersonne['adresse'] = str(sh_demande.cell_value(line, 16)).strip()
                            dataParcelle['categorie'] = str(sh_demande.cell_value(line,17)).strip().upper()
                            dataParcelle['consistance'] = str(sh_demande.cell_value(line, 18)).strip()
                            ...
                            dataParcelle['duree_occupation'] = None
                            if sh_demande.cell_type(line, 30) == 2:
                                dataParcelle['duree_occupation'] = str(int(sh_demande.cell_value(line, 30))).strip()
                            if sh_demande.cell_type(line, 30) == 1:
                                try:
                                    dataParcelle['duree_occupation'] = int(str(sh_demande.cell_value(line, 30)).strip())
                                except Exception:
                                    pass
                            print "duree occupation ***************************************************************************************"
                            print dataParcelle['duree_occupation']
                            print str(sh_demande.cell_value(line, 30)).strip()
                            print "duree occupation ***************************************************************************************"
                            dataParcelle['origine'] = str(sh_demande.cell_value(line, 31)).decode('utf-8').strip()

                            #INFORMATIONS SUR LA RECONNAISSANCE LOCALE
                            dataParcelle['dateReconnaissance'] = self.traiter_date(sh_demande,line,32,wb)
                            print "Date reconnaissance = "
                            print dataParcelle['dateReconnaissance']
                            #dataParcelle['dateReconnaissance'] = str(sh_demande.cell_value(line, 32)).decode('utf-8').strip()
                            dataParcelle['solotena_commune'] = str(sh_demande.cell_value(line, 33)).decode('utf-8').strip().upper()
                            dataParcelle['cin_solotena_commune'] = str(sh_demande.cell_value(line, 34)).decode('utf-8').strip()
                            dataParcelle['signature_solotena_commune'] = str(sh_demande.cell_value(line, 35)).decode('utf-8').strip()
                            dataParcelle['solotena_fkt'] = str(sh_demande.cell_value(line, 36)).decode('utf-8').strip().upper()
                            dataParcelle['cin_solotena_fkt'] = str(sh_demande.cell_value(line, 37)).decode('utf-8').strip()
                            dataParcelle['signature_solotena_fkt'] = str(sh_demande.cell_value(line, 38)).decode('utf-8').strip()
                            dataParcelle['rad1'] = str(sh_demande.cell_value(line, 39)).decode('utf-8').strip().upper()
                            dataParcelle['cin_rad1'] = str(sh_demande.cell_value(line, 40)).decode('utf-8').strip()
                            dataParcelle['signature_rad1'] = str(sh_demande.cell_value(line, 41)).decode('utf-8').strip()
                            dataParcelle['rad2'] = str(sh_demande.cell_value(line, 42)).decode('utf-8').strip().upper()
                            dataParcelle['cin_rad2'] = str(sh_demande.cell_value(line, 43)).decode('utf-8').strip()
                            dataParcelle['signature_rad2'] = str(sh_demande.cell_value(line, 44)).decode('utf-8').strip()
                            dataParcelle['rad3'] = str(sh_demande.cell_value(line, 45)).decode('utf-8').strip().upper()
                            dataParcelle['cin_rad3'] = str(sh_demande.cell_value(line, 46)).decode('utf-8').strip()
                            dataParcelle['signature_rad3'] = str(sh_demande.cell_value(line, 47)).decode('utf-8').strip()
                            dataParcelle['mpiasa_bif'] = str(sh_demande.cell_value(line, 48)).decode('utf-8').strip().upper()
                            dataParcelle['cin_mpiasa_bif'] = str(sh_demande.cell_value(line, 49)).decode('utf-8').strip()
                            dataParcelle['sonia_mpiasa_bif'] = str(sh_demande.cell_value(line, 50)).decode('utf-8').strip()
                            dataParcelle['president_crl'] = str(sh_demande.cell_value(line, 51)).decode('utf-8').strip().upper()
                            dataParcelle['charges'] = str(sh_demande.cell_value(line, 52)).decode('utf-8').strip()
                            dataParcelle['opposition'] = False
                            if sh_demande.cell_value(line, 53) == 1 or str(sh_demande.cell_value(line, 53)).strip().upper() == 'VRAI':
                                dataParcelle['opposition'] = True
                            dataParcelle['avis_crl'] = False
                            if sh_demande.cell_value(line, 55) == 1 or str(sh_demande.cell_value(line, 55)).strip().upper() == 'FAUX':
                                dataParcelle['avis_crl'] = True
                            print "avis CRL**************************************************************************************"
                            print str(sh_demande.cell_value(line, 55)).strip().upper()
                            print dataParcelle['avis_crl']
                            print 'fin avis CRL'

                            dataParcelle['texte_crl'] = ''
                            if str(sh_demande.cell_value(line, 54)).strip() != '':
                                dataParcelle['texte_crl'] =  str(sh_demande.cell_value(line, 54)).strip()
                            print "texte CRL**************************************************************************************"
                            print dataParcelle['texte_crl']


                            #FIN INFORMATIONS SUR LA RECONNAISSANCE LOCALE
                            ...
                            # voisin1:voisin2<FIELD>signature1:signature2 pour chaque point cardinal
                            dic_voisins = {}
                            dic_voisins['nord'] = str(sh_demande.cell_value(line, 22)).decode('utf-8').strip() + '<FIELD>' + str(sh_demande.cell_value(line, 23)).strip()
                            print str(sh_demande.cell_value(line, 23)).strip()
                            #voisins['nord_signature'] =
                            dic_voisins['sud'] = str(sh_demande.cell_value(line, 24)).decode('utf-8').strip() + '<FIELD>' + str(sh_demande.cell_value(line, 25)).strip()
                            #voisins['sud_signature'] =
                            dic_voisins['est'] = str(sh_demande.cell_value(line, 26)).decode('utf-8').strip() + '<FIELD>' + str(sh_demande.cell_value(line, 27)).strip()
                            #voisins['est_signature'] = str(sh_demande.cell_value(line, 27)).decode('utf-8').strip()
                            dic_voisins['ouest'] = str(sh_demande.cell_value(line, 28)).decode('utf-8').strip() + '<FIELD>' + str(sh_demande.cell_value(line, 29)).strip()
                            #voisins['ouest_signature'] = str(sh_demande.cell_value(line, 29)).decode('utf-8').strip()
                            ...
                            voisins, signatures = '', ''
                            for key, val in dic_voisins.items():
                                print "boucle"
                                print val
                                idpoint = None
                                if key == 'nord':
                                ...
                                if toUpdate:
                                    try:
                                        ...
                                        cursor.execute(
                                            ...
                                            , (voisins, str(idpoint), id_parcelle))
                                        self.connection.commit()
                                        print ("mise a joour voisins")
                                        cursor.close()
                                    except Exception as e:
                                        print "Erreur update voisin" + str(e)
                                        self.connection.rollback()

                                #appel de insert to blob voisin
                                print "signatures avant insert"
                                print signatures
                                self.insertIntoBlobVoisin(idpoint, id_parcelle, voisins, signatures)
                            ...
                            try:
                                ...
                                print "Erreur insertion avoir demande " + str(err)
                                self.connection.rollback()
                                cursor.close()

                            #Mise a jour membre RL
                            #Solotena Commune
                            idOlona = self.getIdPersonneFromNameCin(dataParcelle['solotena_commune'], dataParcelle['cin_solotena_commune'])
                            if dataParcelle['solotena_commune'] == dataParcelle['president_crl']:
                                self.updateRL(idOlona,id_demande,2, True)
                            else:
                                self.updateRL(idOlona, id_demande, 2)
                            infoFichier = self.getFileDetails(dataParcelle['signature_solotena_commune'])
                            print infoFichier
                            self.prepareAndInsertFile(infoFichier,dir_path,idOlona)

                            #Fokontany
                            idOlona = self.getIdPersonneFromNameCin(dataParcelle['solotena_fkt'],
                                                                dataParcelle['cin_solotena_fkt'])
                            if dataParcelle['solotena_fkt'] == dataParcelle['president_crl']:
                                self.updateRL(idOlona, id_demande, 3, True)
                            else:
                                self.updateRL(idOlona, id_demande, 3)
                            infoFichier = self.getFileDetails(dataParcelle['signature_solotena_fkt'])
                            print infoFichier
                            self.prepareAndInsertFile(infoFichier, dir_path, idOlona)


                            #RAD1
                            idOlona = self.getIdPersonneFromNameCin(dataParcelle['rad1'],
                                                                dataParcelle['cin_rad1'])
                            if dataParcelle['rad1'] == dataParcelle['president_crl']:
                                self.updateRL(idOlona, id_demande, 4, True)
                            else:
                                self.updateRL(idOlona, id_demande, 4)
                            infoFichier = self.getFileDetails(dataParcelle['signature_rad1'])
                            self.prepareAndInsertFile(infoFichier, dir_path, idOlona)
                            #RAD2
                            idOlona = self.getIdPersonneFromNameCin(dataParcelle['rad2'],
                                                                dataParcelle['cin_rad2'])
                            if dataParcelle['rad2'] == dataParcelle['president_crl']:
                                self.updateRL(idOlona, id_demande, 5, True)
                            else:
                                self.updateRL(idOlona, id_demande, 5)
                            infoFichier = self.getFileDetails(dataParcelle['signature_rad2'])
                            self.prepareAndInsertFile(infoFichier, dir_path, idOlona)
                            #RAD3
                            idOlona = self.getIdPersonneFromNameCin(dataParcelle['rad3'],
                                                                dataParcelle['cin_rad3'])
                            if dataParcelle['rad3'] == dataParcelle['president_crl']:
                                self.updateRL(idOlona, id_demande, 6, True)
                            else:
                                self.updateRL(idOlona, id_demande, 6)
                            infoFichier = self.getFileDetails(dataParcelle['signature_rad3'])
                            self.prepareAndInsertFile(infoFichier, dir_path, idOlona)
                            #BIF
                            idOlona = self.getIdPersonneFromNameCin(dataParcelle['mpiasa_bif'],
                                                                dataParcelle['cin_mpiasa_bif'])
                            if dataParcelle['mpiasa_bif'] == dataParcelle['president_crl']:
                                self.updateRL(idOlona, id_demande, 7, True)
                            else:
                                self.updateRL(idOlona, id_demande, 7)
                            infoFichier = self.getFileDetails(dataParcelle['sonia_mpiasa_bif'])
                            self.prepareAndInsertFile(infoFichier, dir_path, idOlona)

                            #insertion charges
                            res = self.insertCharges(dataParcelle, id_parcelle)

                            if res is not None:
                                id_charge = res[0]
                                try:
                                    curs = self.connection.cursor()
                                    curs.execute("INSERT INTO autrechargesparcelle_d(idparcelle, idcharge) VALUES (%s, %s)",
                                                 (id_parcelle, id_charge))
                                    self.connection.commit()
                                except Exception as err:
                                    print err
                                    self.connection.rollback()

                    dataToLog.append({"code_parcelle": dataParcelle['code_parcelle'], "numero_demande": self.numdemande,
        ...
        self.logger.addSheet(title=title, data=dataToLog, sheet_name="Log Import Data RL")
    ...
    def insertDataDemande(self, data, idparcelle):
        ...
        if self.exists('demande', 'gid', idparcelle):
            ...
            cur = self.connection.cursor()
            try:
                cur.execute('update demande set datereconnaissance = %s, duree_occupation = %s, origine = %s, opposition = %s, avis_crl = %s, texte_crl = %s  where gid = %s and numdemande IS NOT NULL returning iddemande, numdemande', (data['dateReconnaissance'], data['duree_occupation'], data['origine'], data['opposition'], data['avis_crl'], data['texte_crl'],idparcelle))
                res = cur.fetchone()
            ...
            try:
                cur.execute(
                    ...
                    'consistance = %s, idprojet = %s, code_parcelle = %s, categorie = %s, planche_plof = %s, lieudit = %s, collecteur_demande = %s, datereconnaissance = %s, duree_occupation = %s, origine = %s, opposition = %s, avis_crl = %s , texte_crl = %s WHERE gid = %s AND numdemande IS NULL returning iddemande ',
                    (self.region, num_demande, data['date_demande'], data['district'], data['commune'],
                     ...
                     data['collecteur_demande'],data['dateReconnaissance'], data['duree_occupation'], data['origine'], data['opposition'], data['avis_crl'], data['texte_crl'], idparcelle))
                self.connection.commit()
                self.etatInsertion = self.etatInsertion + "MISE A JOUR DEMANDE REUSSIE\n"
                res = cur.fetchone()
                cur.close()
            ...
            try:
                cur.execute(
                    'INSERT INTO demande (region,numdemande, gid, datedemande, district, commune, fokontany, idfokontany, idcommune, consistance, idprojet, code_parcelle, categorie, planche_plof, lieudit, collecteur_demande, datereconnaissance, duree_occupation, origine, opposition, avis_crl , texte_crl ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) returning iddemande ',
                    (self.region,num_demande, idparcelle, data['date_demande'], data['district'], data['commune'], data['fokontany'], self.idFkt, globalvars.id_commune, data['consistance'], globalvars.id_projet,
                     data['code_parcelle'], data['categorie'], data['codePlanche'], data['lieudit'], data['collecteur_demande'], data['dateReconnaissance'], data['duree_occupation'], data['origine'], data['opposition'], data['avis_crl'], data['texte_crl']))
                self.connection.commit()
                self.numdemande = num_demande
                    ...
                    if data['num_copie'] != '':
                        ...
                        if result is not None:
                            ...
                            try:
                                ...
                                return None
            return idpers
        #cas personne non existante en base
        else:
            print "non existe"
            if data['num_pi'] != '':
                print "num_pi"
                try:
                    ...
                    print "Erreur insertion personne " + str(err)
                    #self.emit(SIGNAL("addSubStep(int, QString, QString)"), self.current_step, str(err), "red")
                    self.connection.rollback()
                    self.etatInsertion = self.etatInsertion + "ECHEC INSERTION PERSONNE\n"
                    self.erreur = self.erreur + u"Erreur insertion info sur personne physique\n" + str(err)
                    return None
            else:
                if data['num_copie'] != '':
    ...
    def updateCptDemande(self):
        cursor = self.connection.cursor()
        try:
            cursor.execute("UPDATE commune SET cptdemande = (SELECT (MAX(CAST(TRIM(SPLIT_PART(dmd.numdemande,'-','4')) as integer)) + 1) as ordre FROM demande dmd WHERE TRIM(SPLIT_PART(dmd.numdemande,'-','4')) <> '' ) WHERE idcommune = %s", (globalvars.id_commune,))
            self.connection.commit()
    ...
    def traiter_date(self,sheet, line, col, wb):
        if sheet.cell_type(line, col) == 3 or sheet.cell_type(line,col) == 5:  # verification si la cellule est une date
            ...
            try:
                ...
                print err
                return None
        elif sheet.cell_type(line, col) == 1:
            try:
                date = datetime.strptime(sheet.cell_value(line, col), "%d/%m/%Y")
                return date.date()
            except Exception as err:
                print err
                return None
    ...
    def insertBlobFiles(self, idpersonne, cin_recto = None, cin_verso = None, signature = None, empreinte_d = None, empreinte_g = None, edit_mode = 0, ext_signature = None, ext_cin_recto = None, ext_cin_verso = None, nom_signature = None, nom_cin_recto = None, nom_cin_verso = None):
        ...
        if edit_mode == 0:
            try:
                cur.execute('INSERT INTO blob_personne (idpersonne,cin_recto , cin_verso, signature, empreinte_d, empreinte_g, signature_type, cin_recto_type, cin_verso_type, signature_name, cin_recto_name, cin_verso_name ) '
                            'VALUES (%s, %s, %s, %s, %s,%s,%s,%s,%s,%s,%s,%s) ',
                            (idpersonne, psycopg2.Binary(cin_recto), psycopg2.Binary(cin_verso),psycopg2.Binary(signature), psycopg2.Binary(empreinte_d), psycopg2.Binary(empreinte_g), ext_signature, ext_cin_recto, ext_cin_verso, nom_signature, nom_cin_recto, nom_cin_verso))
                self.connection.commit()
            ...
            try:
                cur.execute('UPDATE blob_personne SET cin_recto = %s, cin_verso = %s, signature = %s, empreinte_d =%s, empreinte_g = %s '
                            'WHERE idpersonne = %s',
                            (psycopg2.Binary(cin_recto), psycopg2.Binary(cin_verso),psycopg2.Binary(signature), psycopg2.Binary(empreinte_d), psycopg2.Binary(empreinte_g), idpersonne))
                self.connection.commit()
    ...
    def importParcelle(self):
        ...
        for f in features:
            ...
            if gid is not None:
                ...
                try:
                    cur = self.connection.cursor()
                    cur.execute('INSERT INTO demande (code_parcelle, gid, idcommune) VALUES (%s, %s, %s)',
                                (codParcel, gid, globalvars.id_commune))
                    self.connection.commit()
                    # cur.close()
                except Exception as err:
                    print err
    ...
    def getIfHasRepresentant(self, idparcelle):
        ...
        yes = False
        try:
            cur.execute("SELECT iddemande FROM avoir_demande WHERE idparcelle = %s and representant = %s", (idparcelle, True))
            res = cur.fetchone()
            if res is not None:
                if len(res) > 0:
                    yes = True
    ...
    def insertIntoBlobVoisin(self, idPoint, idparcelle, voisins, signatures):
        ...
        if voisins != '':
            ...
            if len(tab_voisin) > 0:
                ...
                voisin = voisins
                if len(tab_signature) > 0:
                    signature = tab_signature[0]
                else:
    ...
    def writeBlobInDB(self, idPoint, idparcelle, voisin, infoFic):
        ...
        if infoFic['nom_fic'] !='' and infoFic['path_to'] != '' and infoFic['extension'] !='':
            #preparation du fichier
            signature_file = None
            dir_path = os.path.dirname(str(self.filenames['listing']))
            signature_path = dir_path + infoFic['path_to']
    ...
    def getIdPersonneFromNameCin(self, name, cin = ''):
        if cin != '':
            cur = self.connection.cursor()
            try:
                cur.execute("SELECT idpersonne FROM personne WHERE UPPER(TRIM(CONCAT(nompersonne,' ', COALESCE(prenompersonne,'')))) = %s and numcipersonne = %s", (str(name).strip().upper(), str(cin).strip()))
                res = cur.fetchone()
                return res
            except Exception as err:
                print err
            ...
            try:
                cur.execute(
                    "SELECT idpersonne FROM personne WHERE UPPER(TRIM(CONCAT(nompersonne,' ', COALESCE(prenompersonne,'')))) = %s ",
    ...
    def updateRL(self, idpersonne, iddemande, idrole, president = False):
        if idpersonne is not None:
            print "UPDATE RL " + str(idrole) + " et demande " + str(iddemande) + " et personne " + str(idpersonne[0]) + " et president = " + str(president) + " *******************************************************************"
            cur = self.connection.cursor()
            try:
                cur.execute(
                    "UPDATE demande_crl SET rl =  TRUE, president = %s WHERE idpersonne =  %s and id_role = %s and iddemande = %s",
                    (president, idpersonne[0], idrole, iddemande))
                self.connection.commit()
                print "UPDATEDDDDDD RL " + str(idrole) + " et demande " + str(
                    iddemande) + " *******************************************************************"
            except Exception as err:
                print err
                self.connection.rollback()
    ...
    def prepareAndInsertFile(self, infoFichier, dir_path, idOlona):
        if infoFichier['nom_fic'] != '' and infoFichier['path_to'] != '' and infoFichier[
            'extension'] != '':
            # preparation du fichier
            signature_file = None
            # dir_path = os.path.dirname(str(self.filenames['listing']))
            signature_path = dir_path + infoFichier['path_to']
            if os.path.isfile(signature_path):
                try:
                    signature_file = open(signature_path, 'rb').read()
                    print "fichier signature ouvert"
                    ...
                    print ("Erreur ouverture fichier signature" + err)
            if signature_file is not None:
                self.insertBlobFiles(idpersonne=idOlona, signature=signature_file,
                                     nom_signature=infoFichier['nom_fic'],
                                     ext_signature=infoFichier['extension'])

    def insertCharges(self, data, idparcelle):
        print "call of insert charge"
        print data['charges']
        res = None
        if data['charges'] != '':
            try:
                cur = self.connection.cursor()
                cur.execute("SELECT idcharge FROM autrecharge WHERE descriptioncharge = %s and dateinscriptionregistre = %s and idparcelle = %s"
                            ,(data['charges'], data['dateReconnaissance'], idparcelle))
                res = cur.fetchone()
                if res is not None:
                    return res
            except Exception as err:
            ...
            if res is None:
                try:
                    cur = self.connection.cursor()
                    cur.execute("INSERT INTO autrecharge(type,descriptioncharge,dateinscriptionregistre,idparcelle) "
                                "VALUES ('Charge',%s, %s, %s) returning idcharge", (data['charges'],data['dateReconnaissance'], idparcelle ))
                    self.connection.commit()
                    print "CHARGE INSCRITE"
                    res = cur.fetchone()
                except Exception as err:
                    print err
                    self.connection.rollback()
