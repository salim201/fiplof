...
import datetime
from PopupInfosDemandeFn import Ui_INFORMATIONS
...
class PopupInfosDemandeFnRun(Qt.QDialog):
    ...
    def init_actions(self):
        self.ui.pushButtonEditFisc.clicked.connect(self.demandefn_edit)
        self.ui.pushButtonAnnuler.clicked.connect(self.stop_edit)
        self.ui.pushButtonValider.clicked.connect(self.save_demandefn_edit)

    def getFeatureInfos(self):
        connection = self.connection
        cursor = connection.cursor()
        cursor.execute(
            "SELECT df.fn_fg,df.demandeur,df.sur_plan"
            " FROM demandefn df  WHERE df.gid=%s ", [int(self.gid)])
        dm = cursor.fetchone()
    ...
    def save_demandefn_edit(self):
        try:
            ...
            exe = cursor.execute(
                "update demandefn set fn_fg = %s,demandeur = %s,sur_plan = %s"
                    "WHERE gid=%s ",
                (fn_fg,demandeur,sur_plan,self.gid))
            
            self.connection.commit()
            QtGui.QMessageBox.information(
                    self,
                    "Edition DEMANDE FN", _fromUtf8("Donnée insérer avec succès")
                )
            self.stop_edit()
