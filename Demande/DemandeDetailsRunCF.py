...
class DemandeDetailsRun(QtGui.QDialog):
  ...
  def loadDemandeur(self,connexion):
      ...
      cursor.execute("SELECT * from personne  as d "
  ...
  def updateInfos(self):
    ...
    try:
        cursor.execute(
            "UPDATE demande SET cout=(%s), datedemande= (%s), numdemande=(%s) , datereconnaissance=(%s) , consistance=(%s) , numdemandepaps=(%s)  WHERE gid = (%s)",
            (cout, datetime.date(int(dtDmande[2]), int(dtDmande[1]), int(dtDmande[0])), numDmd,
             datetime.date(int(dtReconnaissance[2]), int(dtReconnaissance[1]), int(dtReconnaissance[0])), consistance,
             numdemandepaps, int(self.gid)))

        cursor.execute("UPDATE parcelle_d SET numdemande=(%s), numerodmdpaps=(%s) , grille=(%s) WHERE gid = (%s)",
                       (numDmd, numdemandepaps, grille, self.gid))
    ...
    try:
        while (i < rowCount):
            ...
            exe = cursor.execute("INSERT INTO avoir_demande (idpersonne,iddemande,idparcelle) VALUES (%s,%s,%s)",
                                 (int(idD), int(self.iddemandeForJournal), self.idparcelle))
            self.connection.commit()
            i = i + 1
    except StandardError as e:
        print e
