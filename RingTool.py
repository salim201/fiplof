...
class RingTool(QgsMapToolIdentify):
    CAPTURE_LINE = 1
    CAPTURE_POLYGON = 2
    finished = pyqtSignal(QgsPoint)
    locationChanged = pyqtSignal(QgsPoint)

    def __init__(self, canvas, layer, onCreateGeomFinished, captureMode, Iface, ifc,sender,parent):  # miampy param iray ahafantaran hoe avy aiz le creation parcelle (Salim)

        QgsMapToolIdentify.__init__(self, canvas)
        self.canvas = canvas
        utils = self.canvas.snappingUtils()
        self.layer = layer
        self.onCreateGeomFinished = onCreateGeomFinished
        ...
        self.feature = None
        self.CurrentGeomTo = self.parent.CurrentGeomTo
    ...
    def snappoint(self, point):
        ...
        if match.isValid():
            ...
            if self.captureMode == RingTool.CAPTURE_LINE:
                self.tempRubberBand.addPoint(tmpMapPt)


            elif self.captureMode == RingTool.CAPTURE_POLYGON:
    ...
    def canvasReleaseEvent(self, event):
        ...
        results = self.identify(event.x(), event.y(), self.TopDownStopAtFirst, [self.layer,fiscalite],
                                self.VectorLayer)
        if len(results) > 0:
            ...
            layer.removeSelection()
            layer.triggerRepaint()
            self.CurrentGeomTo = self.feature.geometry()
            self.CurrentFeatureId = self.feature.id()
        ...
        if event.button() == Qt.LeftButton:
            ...
            #partieValidate = menu.addAction(QIcon("icone/accept.png"), _fromUtf8("Terminer la partie"))
    ...
    def bandType(self):
        ...
        if self.captureMode == RingTool.CAPTURE_POLYGON:
            return QGis.Polygon
        else:
            return QGis.Line
    ...
    def geometryCaptured(self, layerCoords):
        ...
        if reply == QMessageBox.Yes:
            #print "ok"
            #self.CurrentGeomTo = geometry
            self.onCreateGeomFinished(geometry, self.CurrentGeomTo,self.CurrentFeatureId)  # renvoyer le sender vers la fonction onGeometryAdded (Salim)
        self.balayerCanvas()
