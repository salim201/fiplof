...
ICON = QIcon(":/xytools/images/icon.png")
HELP = QIcon(":/xytools/images/help.png")


class XyTools:

    MSG_BOX_TITLE = "XY tools Plugin "
    XY_COMBO_MSG = "Choisir une colonne"

    def __init__(self, iface,canvas,MainWindow,layer,connection,parent):
    ...
    def excelOpen(self):
        ...
        (filename, filter) = QFileDialog.getOpenFileNameAndFilter(self.MainWindow,
        ...
        # now load in layer
        etat = self.loadSheetAsLayer(provider)
        print("---Etat---")
        print(etat)
        if etat == 1 :
            QMessageBox.critical(self.canvas, "Erreur", "Geometry invalide veuillez revoir l ordre de saisie des points")
            return
        if etat == -1 :
            QMessageBox.critical(self.canvas, "Erreur", u"Geometry déjà Existante")
            return
        try :
    ...
    def loadSheetAsLayer(self, provider):
        ...
        try:
            ...
            if self.check_geom_equals(geom.exportToWkt()):
                self.removeMarkers()
                self.canvas.refresh()
                return -1
            else:
                ...
                exe = cursor.execute(
                 "INSERT INTO parcelle_d (numero,geom,surface ,id_commune)VALUES (%s, ST_GeomFromText(%s, "+str(globalvars.EPSG_SCR)+"),ST_Area(%s), %s) returning gid,surface",
                  (num, str(geom.exportToWkt()), str(geom.exportToWkt()), idCommune))
        ...
        #self.canvas.setSelectionColor(QtGui.QColor(str(globalvars.SelectionColor)))
        #layer.select(int(self.idparcelle))
    ...
    def createMemoryLayer(self, crs=None):
        layer = QgsVectorLayer("Point", "...", "memory")
        if crs:
